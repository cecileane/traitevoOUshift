...
```{r}
sessionInfo()$otherPkgs$l1ou$Version
```
...
----------|-----|-----|-----|-----
β<sub>0</sub>, β<sub>1</sub>, ...| yes | no | no | no
α | yes | yes | no | no
...
RE = sqrt_OU_covariance(eModel$tree, alpha=truealpha, 
                        ...
                        normalize.tree.height=T) # needed in l1ou v1.29
...
save(sigma2_table4,file="unknownconfig_sigma2.RData")
...
round(convar,3)
mean(convar) # 0.06254707
sigma2 # 0.06251866
0.1089751/0.06251866 # 1.743081. l1ou v1.29 without normalize.tree.height: mean(convar) was 0.1089751
```
Mean of contrasts are close to zero and the
variance of contrasts is roughly equal to `sigma2`.
...
max(abs(colMeans(contrast_table2))) # 0.001775369: good
round(colMeans(contrast_table2),5)  # last one: 0
convar=apply(contrast_table2,2,ss)
hist(convar, plot=FALSE)
#breaks: 0 .01 .02 .03 .04 .05 .06 .07
#counts:  3   5   1   0   2   10  79
# the 2 contrasts in the .04-.05 range have variance: 0.045 (#38) and 0.049 (#66)
round(convar,3) # last one: exact 0: all last contrasts are 0
smallvar.id = which(convar<0.035)
length(smallvar.id) # 9: 8 internal nodes + root edge. note: 8 shifts...
smallvar.id                  #    7   16   28   37   39   49   59   82  100
round(convar[smallvar.id],3) # .012 .007 .013 .004 .028 .011 .018 .014 .000
mean(convar[which(convar>0.035)]) # 0.06136439: a bit smallish, but good. 0.06168889 if #38 & #66 are excluded.
sigma2 # 0.06251866
```

plotting the nodes will small contrast variance on the tree:
```{r}
plot(eModel$tree, no.margin=T, show.tip.label=F, root.edge=T)
edgelabels("", shift_config, pch=16, frame="n")
nodelabels(101:199, 101:199, cex=0.5, frame="n")
nodelabels(round(convar[smallvar.id],3), smallvar.id+n_tips)
```
